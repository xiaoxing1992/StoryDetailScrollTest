...
class RoleBarrageLayoutManager : RecyclerView.LayoutManager() {
    /**
     * the return value of [layoutView], it means filling view into lane should be over
     */
    private val layoutFinish = -1
    ...
    var verticalCustomGap: Int? = null
    var horizontalCustomGap: Int? = null

    /**
     * the vertical gap of comment view
     */
    //private val verticalGap = 5.dp()
    private val verticalGap: Int
        get() = verticalCustomGap ?: Random.nextInt(8.dp(), 15.dp())

    /**
     * the horizontal gap of comment view
     */
    //private val horizontalGap = 3.dp()
    private val horizontalGap: Int
        get() = horizontalCustomGap ?: Random.nextInt(20.dp(), 60.dp())
    ...
    private fun fillLanes(recycler: RecyclerView.Recycler?, lanes: MutableList<Lane>) {
        ...
        while (hasMoreLane(height - lanes.bottom())) {
            ...
            if (consumeSpace == layoutFinish) break
        }
    }
    ...
    private fun updateLaneIndexAfterRecycle(lanes: List<Lane>, recycleIndex: Int) {
        lanes.forEach {
            if (it.startLayoutIndex > recycleIndex) {
                it.startLayoutIndex--
            }
            if (it.endLayoutIndex > recycleIndex) {
                it.endLayoutIndex--
            }
        }
    }
    ...
    private fun layoutViewByScroll(recycler: RecyclerView.Recycler?, lane: Lane) {
        var view: View? = null
        try {
            view = recycler?.getViewForPosition(adapterIndex)
    ...
    private fun layoutView(recycler: RecyclerView.Recycler?, lanes: MutableList<Lane>): Int {
        ...
        view ?: return layoutFinish
        measureChildWithMargins(view, 0, 0)
        val verticalMargin = (view.layoutParams as? RecyclerView.LayoutParams)?.let { it.topMargin + it.bottomMargin }
            ?: 0
    ...
    private fun getLayoutIndex(adapterIndex: Int): Int {
        val firstChildIndex = getChildAt(0)?.let { (it.layoutParams as RecyclerView.LayoutParams).bindingAdapterPosition }
            ?: 0
